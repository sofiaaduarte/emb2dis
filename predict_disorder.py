...
def parser():
    ...
    parser.add_argument(
        ...
        choices=[
            "ESM2",
            "ProtT5",
        ],  # Later will add ['ProstT5', 'esmc_300m', 'esmc_600m'],
...
def extract_and_validate_sequence(raw_input):
    ...
    if not sequence:
        raise Exception("No valid amino acid characters found in the input.")
...
def main(sequence, language_model, protein_id=None):
    ...
    if verbose:
        device_name = (
            tr.cuda.get_device_name(device) if device.startswith("cuda") else "CPU"
        )
    ...
    print(f"\nGenerating {language_model} embeddings for sequences in: {fasta}")
    results = generate_embeddings_from_fasta(
        fasta_path=fasta, plm=language_model, verbose=verbose, device=device
    )
